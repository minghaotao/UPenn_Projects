...
def covert_time_zone(time):
    ...
    if 'Z' in time:
        ...
        current_time = datetime.strptime(time[:-6], '%Y-%m-%dT%H:%M:%S.%f')
        time_zone = time[-6:]  # Extract +hh:mm or -hh:mm
        current_time -= timedelta(hours=int(time_zone[:3]), minutes=int(time_zone[4:]))

        # Convert time by subtracting 14 hours
    updated_time = current_time - timedelta(hours=14)
    # print(updated_time.strftime('%Y-%m-%dT%H:%M:%S.%fZ'))
    # Return formatted timestamp (assume UTC after conversion)
    return updated_time.strftime('%Y-%m-%dT%H:%M:%S.%fZ')
...
def compare_24_hours(time, answer_time=None):
    ...
    # print(time_dt)
        ...
        # Add 24 hours
    one_day_time = time_dt + timedelta(hours=24)
    one_day_time = one_day_time.strftime('%Y-%m-%dT%H:%M:%S.%fZ')

    # Get the current time in UTC
    now = datetime.now()
    current_time = datetime.strftime(now, f'%Y-%m-%dT%H:%M:%S.%fZ')
    # print(current_time)


    if answer_time:

        # print(answer_time,one_day_time)
